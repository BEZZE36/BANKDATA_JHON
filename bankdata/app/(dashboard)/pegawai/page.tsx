...
// ─── Filter Status (Client Component) ────────────────────────────────────────
'use client';
function FilterStatus({ currentStatus }: { currentStatus: string }) {
  // Implemented inline, actual would use useRouter
  const options = [
...
function DeleteButton({ id, nama }: { id: number; nama: string }) {
  async function handleDelete() {
    if (!confirm(`Hapus data pegawai "${nama}"? Data akan diarsipkan (soft delete).`)) return;
  ...
  return (
    <button
      ...
      className="btn-ghost px-2 py-1 text-xs rounded-lg text-red-500 hover:bg-red-50"
      aria-label="Hapus"
    >
      <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
