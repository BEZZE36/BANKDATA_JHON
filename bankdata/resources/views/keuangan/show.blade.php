...
<div class="flex flex-col md:flex-row gap-6 items-start">
    <div class="bg-white rounded-2xl border p-6 w-full md:w-2/3 space-y-6">
            ...
            <ul class="space-y-1 text-sm">
                @foreach ($keuangan->attachments as $a)
                    <li>
                        ...
                        <a href="{{ Storage::url($a->file_path) }}" target="_blank" class="text-blue-600 hover:underline">{{ $a->original_name }}</a>
                    </li>
                @endforeach
            </ul>
        </div>
        ...
        <div class="pt-4 border-t flex gap-2">
            <a href="{{ route('keuangan.index') }}" class="px-5 py-2.5 bg-slate-100 rounded-lg font-medium hover:bg-slate-200 text-sm hover:-translate-y-0.5 transition-transform">Kembali</a>
        </div>
    </div>
</div>
@endsection
