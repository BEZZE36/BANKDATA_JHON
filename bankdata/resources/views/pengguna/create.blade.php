    ...
    <form method="POST" action="{{ route('pengguna.store') }}" class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div class="col-span-2">
                ...
                <input name="name" value="{{ old('name') }}" required class="mt-1 w-full rounded-lg border-slate-300 focus:ring-emerald-500 focus:border-emerald-500">
                @error('name') <span class="text-xs text-rose-500">{{ $message }}</span> @enderror
            </div>
            <div>
                <label class="text-sm font-medium">Email</label>
                <input name="email" type="email" value="{{ old('email') }}" required class="mt-1 w-full rounded-lg border-slate-300 focus:ring-emerald-500 focus:border-emerald-500">
                @error('email') <span class="text-xs text-rose-500">{{ $message }}</span> @enderror
            </div>
            <div>
                <label class="text-sm font-medium">Unit Kerja</label>
                <input name="unit_kerja" value="{{ old('unit_kerja') }}" class="mt-1 w-full rounded-lg border-slate-300 focus:ring-emerald-500 focus:border-emerald-500">
            ...
            <div>
                ...
                <input name="password" type="password" required class="mt-1 w-full rounded-lg border-slate-300 focus:ring-emerald-500 focus:border-emerald-500">
                @error('password') <span class="text-xs text-rose-500">{{ $message }}</span> @enderror
            </div>
            <div>
                <label class="text-sm font-medium">Konfirmasi Password</label>
                <input name="password_confirmation" type="password" required class="mt-1 w-full rounded-lg border-slate-300 focus:ring-emerald-500 focus:border-emerald-500">
            </div>
            <div>
                ...
                <div class="mt-2 flex flex-wrap gap-3">
                    @foreach($roles as $role)
                        <label class="flex items-center gap-2 border p-2 rounded-lg cursor-pointer hover:bg-slate-50">
                            <input type="checkbox" name="roles[]" value="{{ $role }}" class="rounded text-emerald-600 focus:ring-emerald-500" {{ is_array(old('roles')) && in_array($role, old('roles')) ? 'checked' : '' }}>
        ...
        <div class="flex gap-3 pt-4">
            <button class="px-5 py-2.5 bg-emerald-600 text-white rounded-lg font-medium hover:bg-emerald-700 hover:-translate-y-0.5 transition-transform">Simpan Pengguna</button>
